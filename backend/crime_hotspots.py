...
def _parse_gemini_response(text):
    """Parse Gemini's JSON response, handling markdown code fences and common LLM JSON issues."""
    import re
    if not text:
        return None
    ...
    try:
        ...
        print(f"[Crime Hotspots] JSON parse error after cleanup: {e}")

    # Last resort: try to extract just the JSON array
    match = re.search(r'\[.*\]', cleaned, re.DOTALL)
    if match:
        try:
            return json.loads(match.group())
        except json.JSONDecodeError as e:
            print(f"[Crime Hotspots] JSON extract error: {e}")
...
[
  {"name": "Example Area", "lat": 18.52, "lon": 73.85, "crime_types": ["theft", "robbery"], "risk_level": 7, "description": "Known for late-night robberies near the station."}
    ...
    if data and isinstance(data, list) and len(data) > 0:
        ...
        valid = [h for h in data if isinstance(h, dict) and isinstance(h.get('lat'), (int, float)) and isinstance(h.get('lon'), (int, float)) and h.get('name')]
        if valid:
            with open(CACHE_FILE, "w") as f:
                json.dump(valid, f, indent=2)
            print(f"[Crime Hotspots] Generated and cached {len(valid)} hotspots via Gemini")
            return valid

    print("[Crime Hotspots] Failed to generate valid data from Gemini")
    ...
    if os.path.exists(CACHE_FILE):
        try:
            with open(CACHE_FILE, "r") as f:
                cached = json.load(f)
            # Validate cached data has valid entries
            valid = [h for h in cached if isinstance(h, dict) and isinstance(h.get('lat'), (int, float)) and isinstance(h.get('lon'), (int, float))]
            if valid:
                _crime_data = valid
                print(f"[Crime Hotspots] Loaded {len(_crime_data)} hotspots from cache")
                return _crime_data
            else:
                print("[Crime Hotspots] Cache contains invalid data, regenerating...")
